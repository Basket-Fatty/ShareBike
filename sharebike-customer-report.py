...
def load_main_frame():
    ...
    tk.Button(
        ...
        command=lambda: view_report_button(vehicle_id_box, time_box, status_box, location_id_box)
    ).place(x=180, y=450)


# view_report_function
def view_report_button(vehicle_id_box, time_box, status_box, location_id_box):
    ...
    customer.report(int(vehicle_id), int(time), status, int(location_id))


# initialization
root = tk.Tk()
root.title('ShareBike | Report - Customer')
