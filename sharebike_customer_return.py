...
def config(window, cust_id):
    window.title('ShareBike | Return - Customer')
    ...
    def load_main_frame():
        ...
        tk.Button(
            ...
            command=lambda: view_report_button(loc_selected_option)
        ).place(x=155, y=470)

    # view_report_function
    def view_report_button(loc_selected_option):
        ...
        vehicle_id = dbFun.get_latest_trip(cust_id)[0][1]
        location_id = dbFun.get_loc_id(loc_selected_option.get())
        customer.returnBike(int(vehicle_id), int(location_id), cust_id)
